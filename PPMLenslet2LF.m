% PPMLenslet2LF - decodes a 10-bit ppm 2D lenslet image that has been previously demosaiced and devignetted into a 4D light field
% Based on the LF Toolbox function LFDecodeImageSimple
%
...
%
%   [LF, LFCol] = ...
%      PPMLenslet2LF( LensletImagePath, MetadataPath )
%
% Input LensletImagePath is the absolute or relative path of 10-bit lenslet 
% image that has already been demosaiced and devignetted.
%
% Input MetadataPath is the absolute or relative path of the metadata
% corresponding to the image pointed at by LensletImagePath. The metadata
...
%
% Output LFCol is a 5D array of size [Nj,Ni,Nl,Nk, 3]. LFCol is the color
% and gamma corrected version of LF.
%
% See also:  LFLytroDecodeImage, LFUtilDecodeLytroFolder

function [LF, LFCol] = PPMLenslet2LF( LensletImagePath, MetadataPath )

LensletImage = imread(LensletImagePath);
MD = load(MetadataPath);
DecodeOptions = MD.DecodeOptions;
LensletGridModel = MD.LensletGridModel;


%---Defaults---
% DecodeOptions = LFDefaultField( 'DecodeOptions', 'LevelLimits', []);
...
% end
% 
% LensletImage = uint16(LensletImage .*(2^16-1));

% %--CLIPPING TO 8 BITS HERE--%
% LensletImage = LensletImage .*(2^10-1);
% LensletImage = LensletImage./(2^2); % shifting by 2 bits
% LensletImage = LensletImage ./  double(intmax('uint8'));
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
LF = uint16(LF(:,:,:,:,1:3));
Gamma = 1/2.2;
LFCol = im2uint16(LFColourCorrect(im2double(LF), DecodeOptions.ColourMatrix, DecodeOptions.ColourBalance, Gamma));
